...
export class SwRefreshListView extends ListView{
  ...
  beginRefresh(){
    ...
    if (!this._isRefreshing){
      ...
      this.scrollToFix({x:0,y:-height,animated:true});
...
//----------------------------------------------
  ...
  componentDidMount(){
    ...
    this.initializationTimer = setTimeout(()=>{
      this.scrollToFix({x: 0, y: height, animated: false})
    },20)
  ...
  _onScrollEndDrag(event,inertia){
    ...
    if (!this._isRefreshing){
      if (this.state.refresStatus == RefreshStatus.releaseToRefresh){
        ...
        this.scrollToFix({x:0,y:0,animated:true});
        if(this.props.onRefresh){
          this.props.onRefresh(()=>{
            this._onRefreshEnd()
          })
      ...
      if (y <= height) {
        this.scrollToFix({x:0,y:0,animated:!inertia});
      }
    }
  ...
  scrollToFix(param){
    if(this && this.refs.listView && this.refs.listView.scrollTo){
      this.refs.listView.scrollTo(param);
    } else {
      __DEV__ && console.log('swrefresh scroll to crash')
    }
  }
